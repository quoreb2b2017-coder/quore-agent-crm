-- Paste in Supabase SQL Editor. Safe to run again if the last attempt failed.
-- Domain-level website activity from the Chrome extension.
-- Does not call has_role() — some projects never created that helper.
...
stable
as $$
  select public.jwt_role_key() = any (
...
grant execute on function public.is_admin_like() to authenticated, anon, service_role;
...
create index if not exists idx_user_activities_user_started on public.user_activities (user_id, started_at desc);
create index if not exists idx_user_activities_employee_started on public.user_activities (employee_id, started_at desc);
create index if not exists idx_user_activities_domain on public.user_activities (domain);
create index if not exists idx_user_activities_created on public.user_activities (created_at desc);
create index if not exists idx_user_activities_started on public.user_activities (started_at desc);
